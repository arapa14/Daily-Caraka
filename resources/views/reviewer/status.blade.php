...
<body>
    ...
    <div class="bg-white shadow-md rounded-lg p-4 flex flex-col sm:flex-row justify-between items-center">
        ...
        <a href="{{ route('dashboard') }}"
            class="text-xl font-bold text-blue-600 mb-2 sm:mb-0 sm:text-lg hover:bg-blue-50 rounded-lg px-2 py-2 cursor-pointer">Laporan</a>
                ...
                <tbody class="bg-white">
                    @foreach ($laporans as $laporan)
                        <tr class="border-b hover:bg-gray-50 text-center transition duration-200">
                            ...
                            <td class="p-3">
                                <form action="{{ route('update.status', $laporan->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <select name="status"
